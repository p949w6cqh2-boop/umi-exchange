...
- **In-flight (`fix/consent-voice`, 2026-07-14):** triage items 1–14 verified all shipped in `fc7f7e5`; the two remainders done — consent FILLED state speaks parish (scope tokens → words, Stop sharing, honest footnote) and the gallery re-captured under real `DEBUG=0` demo conditions (recipe verified live: warm 404, whitenoise statics). Gate pending, then his gallery key.
