...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--sleep", type=float, default=0.0,
                            help="Seconds between batches (be kind to prod).")
        parser.add_argument("--dry-run", action="store_true")
        parser.add_argument("--verify", action="store_true",
                            help="Read-only census: legacy/envelope/unreadable.")
        parser.add_argument("--to-legacy", action="store_true",
                            help="Emergency reverse for stage-2 rollback.")
    ...
    def handle(self, *args, **opts):
        ...
        pending = (base.filter(on_behalf_of_dek__isnull=False) if reverse
                   else base.filter(on_behalf_of_dek__isnull=True))

        if opts["dry_run"]:
            self.stdout.write(f"{pending.count()} row(s) would be "
                              f"{'reverted' if reverse else 'backfilled'}.")
            return
        ...
        while True:
            with transaction.atomic():
                rows = list(
                    pending.select_for_update(skip_locked=True)
                    [: opts["batch_size"]])
                if not rows:
                    break
                for need in rows:
                    try:
                        if reverse:
                            plain = crypto.envelope_decrypt_str(
                                need.on_behalf_of, need.on_behalf_of_dek)
                            need.on_behalf_of = crypto.encrypt_str(plain)
                            need.on_behalf_of_dek = None
                        else:
                            plain = crypto.decrypt_str(need.on_behalf_of)
                            (need.on_behalf_of,
                             need.on_behalf_of_dek) = \
                                crypto.envelope_encrypt_str(plain)
                        need.save(update_fields=["on_behalf_of",
                                                 "on_behalf_of_dek"])
                        total += 1
        ...
        self.stdout.write(self.style.SUCCESS(
            f"{'Reverted' if reverse else 'Backfilled'} {total} row(s)."))
        if failed:
            self.stderr.write(self.style.WARNING(
                f"{len(failed)} row(s) could not be decrypted and were "
                f"left untouched — investigate before stage 4:"))
    ...
    def _verify(self):
        ...
        for need in (Need.objects
                     .only("id", "on_behalf_of", "on_behalf_of_dek")
                     .iterator()):
            ...
            try:
                if need.on_behalf_of_dek:
                    crypto.envelope_decrypt_str(need.on_behalf_of,
                                                need.on_behalf_of_dek)
                    envelope += 1
                else:
                ...
                unreadable += 1
        self.stdout.write(
            f"empty={empty}  legacy={legacy}  envelope={envelope}  "
            f"unreadable={unreadable}")
        if unreadable:
            self.stderr.write(self.style.WARNING(
                "Unreadable rows exist — wrong/retired KEK or corrupt data."))
        elif legacy == 0:
            self.stdout.write(self.style.SUCCESS(
                "All populated rows are envelopes — stage 4 (cleanup) is safe."))
