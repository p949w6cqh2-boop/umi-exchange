...
PRIMARY_NEEDS = [
    ("food", "Food"), ("rent", "Rent"), ("utilities", "Utilities"),
    ("transportation", "Transportation"), ("medical", "Medical"),
...
class CaseCreateForm(forms.Form):
    ...
    person = forms.ModelChoiceField(
        ...
        label="Existing person", empty_label="— choose —")
    new_person_name = forms.CharField(
        required=False, max_length=200, label="…or new person: full name")
    new_person_contact = forms.CharField(
        required=False, max_length=200, label="Contact (phone or email)")

    assigned_to = forms.ModelChoiceField(
        queryset=Member.objects.none(), required=False, label="Assign to")
    ...
    primary_needs = forms.MultipleChoiceField(
        choices=PRIMARY_NEEDS, required=False,
        widget=forms.CheckboxSelectMultiple)
    intake_date = forms.DateField(
        initial=timezone.localdate,
        widget=forms.DateInput(attrs={"type": "date"}, format="%Y-%m-%d"))
    physical_ref = forms.CharField(required=False, max_length=100,
                                   label="Paper file reference")
    summary = forms.CharField(required=False, max_length=4000,
    ...
    consent_mode = forms.ChoiceField(choices=CONSENT_MODES, widget=forms.RadioSelect,
                                     initial="record")
    existing_consent = forms.ModelChoiceField(
        queryset=Consent.objects.none(), required=False, empty_label="— choose —")
    record_method = forms.ChoiceField(
        choices=[("verbal", "Verbal"), ("paper", "Paper (filed)")], required=False)
    include_export = forms.BooleanField(
        required=False, label="Consent also covers data export (case_export)")
    ...
    def __init__(self, community, member, *args, **kwargs):
        ...
        self.fields["person"].queryset = Person.objects.filter(
            created_in_community=community, merged_into__isnull=True)
        ...
        user_ids = Member.objects.filter(
            community=community, is_active=True).values_list("user_id", flat=True)
    ...
    def clean(self):
        ...
        if bool(person) == bool(new_name):
            raise forms.ValidationError(
                "Choose an existing person OR enter a new person's name (not both).")
        ...
        if mode == "existing":
            ...
            if not consent:
                ...
                if not _consent_is_active(consent):
                    ...
                    self.add_error("existing_consent",
                                   "That consent does not cover case records.")
                elif person and person.linked_user_id:
                    if consent.participant_id != person.linked_user_id:
                        self.add_error("existing_consent",
                                       "That consent belongs to a different person.")
                elif not person or not person.linked_user_id:
                    self.add_error(
                        "existing_consent",
                        "This person has no account — record verbal/paper consent instead.")
        elif mode == "record":
            ...
            if len((data.get("emergency_justification") or "").strip()) < 10:
                self.add_error("emergency_justification",
                               "A brief justification is required (audited).")
        return data
...
class NoteForm(forms.Form):
    ...
    duration_minutes = forms.IntegerField(required=False, min_value=1, max_value=600,
                                          label="Duration (min)")
    location_kind = forms.ChoiceField(choices=CaseNote.LOCATION_CHOICES, initial="home")
    actions = forms.MultipleChoiceField(
        choices=CaseNote.ACTIONS, required=False,
        widget=forms.CheckboxSelectMultiple, label="Quick actions")
    aid_amount = forms.DecimalField(required=False, min_value=0,
                                    max_digits=8, decimal_places=2,
                                    label="Aid value ($)")
    co_visitor = forms.ModelChoiceField(queryset=Member.objects.none(),
                                        required=False, label="Co-visitor")
    body = forms.CharField(widget=forms.Textarea(attrs={
        "rows": 4, "placeholder": "What happened, what was agreed…"}),
        max_length=8000, label="Notes")
    client_uuid = forms.UUIDField(required=False, widget=forms.HiddenInput)

    def __init__(self, community, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["co_visitor"].queryset = Member.objects.filter(
            community=community, is_active=True)
...
class VisitForm(NoteForm):
    ...
    def __init__(self, community, case_qs, *args, **kwargs):
        ...
        self.fields["case"].label_from_instance = (
            lambda c: f"{c.short_code} · {c.subject_person.initials}")
...
class FollowUpForm(forms.Form):
    title = forms.CharField(
        max_length=200,
        help_text="Keep it non-sensitive — no names (e.g. “Check in re: utility bill”).")
    detail = forms.CharField(required=False, max_length=4000,
...
class AssignForm(forms.Form):
    to_member = forms.ModelChoiceField(queryset=Member.objects.none(),
                                       label="Hand off to")
    summary = forms.CharField(
        min_length=10, max_length=2000,
        widget=forms.Textarea(attrs={
            "rows": 3,
            "placeholder": "What the incoming visitor needs to know first…"}),
        label="Handoff summary (required — it becomes their first screen)")

    def __init__(self, community, exclude_member=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        qs = Member.objects.filter(community=community, is_active=True,
                                   role__in=("coordinator", "admin"))
...
class GrantForm(forms.Form):
    ...
    role = forms.ChoiceField(choices=[("viewer", "Viewer (read only)"),
                                      ("contributor", "Contributor (can add notes)")])
    reason = forms.CharField(max_length=200,
                             help_text="Why this person needs access (audited).")
