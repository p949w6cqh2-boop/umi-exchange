...
def followup_overdue_digest():
    ...
    overdue = (FollowUp.objects.filter(status="open", due_date__lt=today)
    ...
    for member, items in by_assignee.items():
        ...
        if Notification.objects.filter(
                recipient=user, type="followup_overdue",
        ...
        lines = [f"• {fu.title} — due {fu.due_date} (case {fu.case.short_code})"
                 for fu in items[:20]]
        body = "\n".join(lines)
        notify(user, "followup_overdue",
               title=f"{len(items)} overdue follow-up(s)", body=body, link=link)
        if user.email:
            try:
                send_mail(
...
def register_schedule():
    ...
    Schedule.objects.update_or_create(
        ...
        defaults={"func": "apps.casework.tasks.followup_overdue_digest",
