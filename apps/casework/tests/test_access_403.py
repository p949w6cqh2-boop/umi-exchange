...
def test_viewer_grant_reads_but_cannot_contribute(world, auth, u):
    ...
    CaseAccessGrant.objects.create(
        case=case, member=world.plain, granted_by=world.coordinator, role="viewer", reason="covering visits"
    )
    client = auth(world.plain_u)
    assert client.get(u("detail", pk=case.pk)).status_code == 200
    resp = client.post(
        u("note-create", pk=case.pk),
        {"kind": "visit", "occurred_at": "2026-06-12T10:00", "location_kind": "home", "body": "hello"},
    )
