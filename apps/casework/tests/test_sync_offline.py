...
def test_sync_creates_finalized_notes(world, auth, u):
    ...
    drafts = [_draft(world, occurred_at=f"2026-06-1{i}T09:00:00Z") for i in (0, 1, 2)]
...
def test_sync_flags_same_hour_duplicates(world, auth, u):
    client = auth(world.coord_u)
    drafts = [_draft(world, occurred_at="2026-06-12T10:00:00Z"), _draft(world, occurred_at="2026-06-12T10:20:00Z")]
    results = client.post(u("sync"), {"drafts": drafts}, **SYNC_KW).json()["results"]
...
def test_sync_forbidden_case_errors_per_item(world, auth, u):
    ...
    results = client.post(u("sync"), {"drafts": [_draft(world)]}, **SYNC_KW).json()["results"]
