...
def test_happy_path_open_note_finalize_followup_close(world, auth, u):
    ...
    resp = client.post(u("create"), {
        "new_person_name": "Jo Doe", "new_person_contact": "555-0199",
        "sensitivity": "standard", "intake_date": timezone.localdate().isoformat(),
        "physical_ref": "Binder 3", "summary": "Initial intake.",
    ...
    resp = client.post(u("note-create", pk=case.pk), {
        ...
        "location_kind": "home", "duration_minutes": "30",
        "actions": ["food_provided", "prayer"], "aid_amount": "40.00",
    ...
    # 3) finalize it
    assert client.post(
        u("note-finalize", pk=case.pk, note_id=note.pk)).status_code in (200, 302)
    note.refresh_from_db()
    assert note.status == "final" and note.finalized_at

    # 4) follow-up → done
    resp = client.post(u("followup-create", pk=case.pk), {
        "title": "Check in re: pantry", "detail": "She asked about Tuesdays.",
        "due_date": timezone.localdate().isoformat(),
        "assigned_to": str(world.coordinator.pk)})
    assert resp.status_code in (200, 302)
    fu = FollowUp.objects.get(case=case)
    assert client.post(u("followup-status", pk=fu.pk),
                       {"status": "done"}).status_code in (200, 302)
    fu.refresh_from_db()
    assert fu.status == "done"

    # 5) close, then admin reopen
    assert client.post(u("status", pk=case.pk),
                       {"status": "closed"}).status_code in (200, 302)
    case.refresh_from_db()
    assert case.status == "closed" and case.closed_at

    assert client.post(u("status", pk=case.pk),
                       {"status": "open"}).status_code == 403  # coordinator
    admin_client = auth(world.admin_u)
    assert admin_client.post(u("status", pk=case.pk),
                             {"status": "open"}).status_code in (200, 302)
    case.refresh_from_db()
...
def test_warm_handoff_gates_case_detail(world, auth, u):
    admin_client = auth(world.admin_u)
    resp = admin_client.post(u("assign", pk=world.case.pk), {
        "to_member": str(world.coordinator2.pk),
        "summary": "Maria prefers afternoon visits; rent is the live issue."})
    assert resp.status_code == 302
    ...
    assert incoming.post(
        u("handoff-ack", pk=world.case.pk, handoff_id=ho.pk)).status_code == 302
    ho.refresh_from_db()
...
def test_only_receiver_can_acknowledge(world, auth, u):
    ...
    assert other.post(
        u("handoff-ack", pk=world.case.pk, handoff_id=ho.pk)).status_code == 403
...
def test_finalize_twice_returns_409(world, auth, u, make_note):
    ...
    client = auth(world.coord_u)
    assert client.post(
        u("note-finalize", pk=world.case.pk, note_id=note.pk)).status_code in (200, 302)
...
def test_create_without_consent_choice_blocked(world, auth, u):
    ...
    resp = client.post(u("create"), {
        "new_person_name": "No Consent", "sensitivity": "standard",
