...
def test_summary_detail_round_trip(world):
    ...
    assert CaseFile.objects.get(pk=world.case.pk).summary == \
        "Family of four; rent arrears."

    ho = WarmHandoff(case=world.case, from_member=world.coordinator,
                     to_member=world.coordinator2)
    ho.summary = "Prefers afternoons."
    ho.save()
    assert WarmHandoff.objects.get(pk=ho.pk).summary == "Prefers afternoons."

    fu = FollowUp(case=world.case, created_by=world.coordinator,
                  assigned_to=world.coordinator, title="Call back",
...
def _actions(resource):
    return list(AuditLog.objects.filter(resource_id=resource.pk)
...
def test_case_opened_carries_consent_id(world, auth, u):
    ...
    client.post(u("create"), {
        "person": str(world.person.pk), "sensitivity": "standard",
        "intake_date": timezone.localdate().isoformat(),
        "consent_mode": "existing", "existing_consent": str(world.consent.pk)})
...
def test_emergency_open_is_flagged_and_audited(world, auth, u):
    client = auth(world.coord_u)
    client.post(u("create"), {
        "new_person_name": "After Hours", "sensitivity": "standard",
        "intake_date": timezone.localdate().isoformat(),
    ...
    row = AuditLog.objects.get(action="case.opened_emergency",
                               resource_id=case.pk)
...
def test_case_viewed_is_throttled_per_session(world, auth, u):
    ...
    assert AuditLog.objects.filter(action="case.viewed",
                                   resource_id=world.case.pk).count() == 1
...
def test_lifecycle_events_emitted(world, auth, u, make_note):
    ...
    assert AuditLog.objects.filter(action="note.finalized",
                                   resource_id=note.pk).exists()

    admin_client = auth(world.admin_u)
    admin_client.post(u("assign", pk=world.case.pk), {
        "to_member": str(world.coordinator2.pk),
        "summary": "Maria prefers afternoons."})
    assert AuditLog.objects.filter(action="case.assigned",
                                   resource_id=world.case.pk).exists()
    ...
    assert AuditLog.objects.filter(action="handoff.acknowledged",
                                   resource_id=ho.pk).exists()


def test_grant_events_and_export_scope(world, auth, u):
    admin_client = auth(world.admin_u)
    admin_client.post(u("grant-create", pk=world.case.pk), {
        "member": str(world.plain.pk), "role": "viewer",
        "reason": "Covering Saturday visits"})
    grant = CaseAccessGrant.objects.get(case=world.case, member=world.plain)
    assert AuditLog.objects.filter(action="grant.granted",
                                   resource_id=grant.pk).exists()

    admin_client.post(u("grant-revoke", pk=world.case.pk, grant_id=grant.pk))
    assert AuditLog.objects.filter(action="grant.revoked",
                                   resource_id=grant.pk).exists()

    resp = admin_client.get(u("export", pk=world.case.pk))
    assert resp.status_code == 200
    assert b"CASE EXPORT" in resp.content
    row = AuditLog.objects.get(action="case.exported",
                               resource_id=world.case.pk)
    assert "case_export" in row.details["scope"]


def test_all_emitted_actions_fit_widened_column(world):
    assert all(len(a) <= 32 for a in
...
def test_revoked_consent_freezes_notes_and_export(world, auth, u):
    ...
    client = auth(world.coord_u)
    resp = client.post(u("note-create", pk=world.case.pk), {
        "kind": "visit", "occurred_at": "2026-06-12T10:00",
        "location_kind": "home", "body": "should be frozen"})
    ...
    assert client.post(u("status", pk=world.case.pk),
                       {"status": "closed"}).status_code in (200, 302)
