...
def test_consent_covers_authorization_check(world):
    ...
    assert legacy.covers(grantee_type="community", grantee_id=world.community.id, scopes=["case_records"])
    assert not legacy.covers(grantee_type="organization", grantee_id=world.community.id, scopes=["case_records"])
    assert not legacy.covers(
        grantee_type="community", grantee_id=world.community.id, scopes=["case_records", "not_granted_scope"]
    )

    other = Consent.objects.create(
        participant=world.subject_u,
        granted_to="Some Other Conference",
        scope=["case_records"],
        purpose="t",
        method="digital",
    ...
    assert not other.covers(grantee_type="community", grantee_id=world.community.id, scopes=["case_records"])

    legacy.status = "revoked"
    legacy.revoked_at = timezone.now()
    legacy.save(update_fields=["status", "revoked_at"])
    assert not legacy.covers(grantee_type="community", grantee_id=world.community.id, scopes=["case_records"])
...
def _make_need(world, title, description=""):
    ...
    category, _ = Category.objects.get_or_create(
        community=world.community, name="Repairs", defaults={"icon": "🔧", "sort_order": 1}
    )
    base = dict(
        community=world.community,
        requester=world.coordinator,
        category=category,
        title=title,
    ...
    try:
        ...
        base.update(status="open", contact_pref="in_app", expires_at=timezone.now() + timedelta(days=30))
        return Need.objects.create(**base)
...
def test_apply_search_matches_and_excludes(world):
    ...
    need = _make_need(world, "Repairing the leaky faucet", "Kitchen faucet drips constantly.")
...
@pytest.mark.skipif(connection.vendor != "postgresql", reason="stemmed FTS is Postgres-only by design (§10.4)")
...
def _make_proposed_match(world, days_old):
    ...
    match = Match.objects.create(need=need, proposed_by=world.plain, status="proposed")
    Match.objects.filter(pk=match.pk).update(proposed_at=timezone.now() - timedelta(days=days_old))
...
def test_sweep_expires_audits_and_notifies_once(world):
    ...
    assert AuditLog.objects.filter(action="match.expired", resource_id=stale.pk).exists()
    assert Notification.objects.filter(recipient=world.plain_u, type="match_expired").count() == 1
