...
def _conflict(exc_or_msg) -> HttpResponse:
    msg = ("; ".join(exc_or_msg.messages)
           if isinstance(exc_or_msg, TransitionConflict) else str(exc_or_msg))
...
class CommunityMixin(LoginRequiredMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.community = get_object_or_404(
            Community, slug=kwargs["slug"], is_active=True)
    ...
    def get_case(self, pk) -> CaseFile:
        return get_object_or_404(
            CaseFile.objects.select_related(
                "subject_person", "assigned_to", "opened_by", "consent"),
            pk=pk, community=self.community)

    def contributable_cases(self):
        """Cases this member may ADD NOTES to (visit page + sync)."""
        m = self.membership
        qs = CaseFile.objects.filter(
            community=self.community, status__in=("open", "monitoring"))
        if not access.is_admin(m):
            now = timezone.now()
            grant_q = (Q(grants__member=m, grants__role="contributor",
                         grants__revoked_at__isnull=True)
                       & (Q(grants__expires_at__isnull=True)
                          | Q(grants__expires_at__gt=now)))
            role_q = (Q(sensitivity="standard")
                      if access.is_coordinator(m) else Q(pk=None))
            qs = qs.filter(Q(assigned_to=m) | Q(opened_by=m)
                           | role_q | grant_q).distinct()
        return (qs.select_related("subject_person")
                  .annotate(prio=SqlCase(When(assigned_to=m, then=Value(0)),
                                         default=Value(1),
...
class CaseListView(CommunityMixin, View):
    def get(self, request, slug):
        ...
        qs = (CaseFile.objects.filter(community=self.community)
              .select_related("subject_person", "assigned_to"))
        if not access.is_admin(m):
            grant_q = (Q(grants__member=m, grants__revoked_at__isnull=True)
                       & (Q(grants__expires_at__isnull=True)
                          | Q(grants__expires_at__gt=now)))
            qs = qs.filter(Q(sensitivity="standard") | Q(assigned_to=m)
                           | Q(opened_by=m) | grant_q).distinct()
        ...
        if q:  # encrypted names are unsearchable by design — code/paper ref only
            ...
            cases = [c for c in cases
                     if qu in c.short_code or q.lower() in c.physical_ref.lower()]

        rows = [{"case": c,
                 "name": c.subject_person.display_name or c.subject_person.short_code}
                for c in cases]
        template = ("casework/_case_rows.html" if _is_htmx(request)
                    else "casework/list.html")
        return render(request, template, {
            "community": self.community, "membership": m, "rows": rows,
...
class CaseCreateView(CommunityMixin, View):
    def get(self, request, slug):
        ...
        return render(request, "casework/create.html",
                      {"community": self.community, "form": form})
    ...
    def post(self, request, slug):
        ...
        if not form.is_valid():
            return render(request, "casework/create.html",
                          {"community": self.community, "form": form}, status=200)
        d = form.cleaned_data
        with transaction.atomic():
            person = d["person"]
            if not person:
                person = Person(created_in_community=self.community,
                ...
                person.save()
                audit.emit("person.created", person,
            ...
            if d["consent_mode"] == "existing":
                ...
                participant = (person.linked_user
                               if person.linked_user_id else request.user)
                scope = ["case_records"] + (
                    ["case_export"] if d["include_export"] else [])
                consent = Consent.objects.create(
                    ...
                    custom=({} if person.linked_user_id
                            else {"on_behalf_person_id": str(person.id)}),
                )
            else:
                emergency = True

            case = CaseFile(
                community=self.community, subject_person=person,
                opened_by=self.membership,
                assigned_to=d["assigned_to"] or self.membership,
                sensitivity=d["sensitivity"], consent=consent,
                emergency_opened=emergency,
                emergency_justification=(d["emergency_justification"].strip()
                                         if emergency else ""),
                primary_needs=d["primary_needs"], intake_date=d["intake_date"],
            ...
            if emergency:
                audit.emit("case.opened_emergency", case, user=request.user,
                           request=request,
                           details={"justification":
                                    case.emergency_justification[:500]})
            else:
                audit.emit("case.opened", case, user=request.user,
                           request=request,
                           details={"consent_id": str(consent.pk)})

            if case.assigned_to and case.assigned_to != self.membership:
                notify(case.assigned_to.user, "case_assigned",
                       f"Case {case.short_code} assigned to you",
                       "A new case file was assigned to you.",
                       reverse("casework:detail",
                               kwargs={"slug": slug, "pk": case.pk}))
...
class CaseDetailView(CommunityMixin, View):
    def get(self, request, slug, pk):
        ...
        pending = case.handoffs.filter(
            to_member=self.membership, status="pending").first()
        if pending:
            return render(request, "casework/handoff_ack.html", {
                "community": self.community, "case": case, "handoff": pending})
        ...
        notes = (case.notes.exclude(status=CaseNote.STATUS_DISCARDED)
                 .select_related("author", "co_visitor", "amends"))
        visible_notes = [n for n in notes
                         if n.status == CaseNote.STATUS_FINAL
                         or n.author_id == self.membership.id]

        can_contribute = level >= access.CONTRIBUTOR
        can_assign = (access.is_admin(self.membership)
                      or case.assigned_to_id == self.membership.id)
        can_export = (access.is_admin(self.membership)
                      and case.consent_id and case.consent_is_active()
                      and "case_export" in (case.consent.scope or []))

        return render(request, "casework/detail.html", {
            "community": self.community, "membership": self.membership,
            "case": case, "level": level,
            "person_name": case.subject_person.display_name
                            or case.subject_person.short_code,
            "notes": visible_notes,
            "followups": case.followups.filter(status="open")
                          .select_related("assigned_to"),
            "grants": case.grants.filter(revoked_at__isnull=True)
                       .select_related("member"),
            "handoffs": case.handoffs.select_related(
                "from_member", "to_member")[:5],
            "note_form": NoteForm(self.community),
            "followup_form": FollowUpForm(self.community),
            "assign_form": AssignForm(self.community,
                                      exclude_member=case.assigned_to),
            "grant_form": GrantForm(self.community),
            "can_contribute": can_contribute, "can_assign": can_assign,
            "can_grant": can_assign, "can_export": can_export,
...
class CaseStatusView(CommunityMixin, View):
    def post(self, request, slug, pk):
        ...
        audit.emit("case.reopened" if reopen else "case.status_changed",
                   case, user=request.user, request=request,
                   details={"to": new})
        if _is_htmx(request):
            return render(request, "casework/_case_status.html", {
                "community": self.community, "case": case,
                "is_admin": access.is_admin(self.membership)})
        return redirect("casework:detail", slug=slug, pk=case.pk)
...
class CaseAssignView(CommunityMixin, View):
    def post(self, request, slug, pk):
        ...
        form = AssignForm(self.community, exclude_member=case.assigned_to,
                          data=request.POST)
        if not form.is_valid():
            return HttpResponse(form.errors.as_text(), status=400)
        to_member = form.cleaned_data["to_member"]
        with transaction.atomic():
            ho = WarmHandoff(case=case, from_member=self.membership,
                             to_member=to_member)
            ho.summary = form.cleaned_data["summary"]
            ho.save()
            sysnote = CaseNote(case=case, author=self.membership, kind="handoff",
                               occurred_at=timezone.now(),
                               status=CaseNote.STATUS_FINAL,
                               finalized_at=timezone.now())
            sysnote.body = (f"Case handed off from {self.membership.display_name} "
                            f"to {to_member.display_name}.")
            sysnote.save()
            case.assigned_to = to_member
            case.save(update_fields=["assigned_to", "updated_at"])
            audit.emit("case.assigned", case, user=request.user, request=request,
                       details={"to_member": str(to_member.pk)})
            notify(to_member.user, "handoff_pending",
...
class HandoffAckView(CommunityMixin, View):
    def post(self, request, slug, pk, handoff_id):
        ...
        try:
            ...
            return _conflict(exc)
        audit.emit("handoff.acknowledged", handoff,
                   user=request.user, request=request)
        return redirect("casework:detail", slug=slug, pk=case.pk)
...
class NoteCreateView(CommunityMixin, View):
    def post(self, request, slug, pk):
        ...
        if self._consent_frozen(case):
            return _forbidden("Consent was revoked — new notes are frozen; "
                              "close the case via status instead.")
        form = NoteForm(self.community, request.POST)
        if not form.is_valid():
            return HttpResponse(form.errors.as_text(), status=400)
        d = form.cleaned_data
        note = CaseNote(case=case, author=self.membership, kind=d["kind"],
                        occurred_at=d["occurred_at"],
                        duration_minutes=d["duration_minutes"],
                        location_kind=d["location_kind"], actions=d["actions"],
    ...
    def _respond(self, request, slug, case, note):
        if _is_htmx(request):
            return render(request, "casework/_note.html", {
                "community": self.community, "case": case, "note": note,
                "membership": self.membership, "can_contribute": True})
        return redirect("casework:detail", slug=slug, pk=case.pk)
...
class NoteFinalizeView(CommunityMixin, View):
    def post(self, request, slug, pk, note_id):
        ...
        audit.emit(action, note, user=request.user, request=request,
                   details=({"amends": str(note.amends_id)}
                            if note.amends_id else None))
        if _is_htmx(request):
            return render(request, "casework/_note.html", {
                "community": self.community, "case": case, "note": note,
                "membership": self.membership, "can_contribute": True})
...
class NoteAmendView(CommunityMixin, View):
    def post(self, request, slug, pk, note_id):
        ...
        if original.status != CaseNote.STATUS_FINAL:
            return _conflict("Only finalized notes can be amended "
                             "(discard drafts instead).")
        body = (request.POST.get("body") or "").strip()
        if not body:
            return HttpResponse("Amendment text is required.", status=400)
        note = CaseNote(case=case, author=self.membership, kind=original.kind,
                        occurred_at=timezone.now(),
                        location_kind=original.location_kind, amends=original)
        note.body = body
        note.save()
        if "finalize" in request.POST:
            note.transition_to(CaseNote.STATUS_FINAL)
            audit.emit("note.amended", note, user=request.user, request=request,
                       details={"amends": str(original.pk)})
        if _is_htmx(request):
            return render(request, "casework/_note.html", {
                "community": self.community, "case": case, "note": note,
                "membership": self.membership, "can_contribute": True})
...
class VisitCaptureView(CommunityMixin, View):
    def get(self, request, slug):
        ...
        return render(request, "casework/visit.html", {
            "community": self.community, "form": form,
    ...
    def post(self, request, slug):
        ...
        try:
            ...
            return redirect(f"{reverse('casework:visit', kwargs={'slug': slug})}"
                            f"?saved={case.short_code}")
        if "finalize" in request.POST:
            note.transition_to(CaseNote.STATUS_FINAL)
            audit.emit("note.finalized", note, user=request.user, request=request)
        return redirect(f"{reverse('casework:visit', kwargs={'slug': slug})}"
...
class VisitManifestView(CommunityMixin, View):
    ...
    def get(self, request, slug):
        cases = [{"id": str(c.id), "code": c.short_code,
                  "initials": c.subject_person.initials}
...
class SyncView(CommunityMixin, View):
    ...
    def _one(self, request, slug, item):
        ...
        if not cu:
            return {"client_uuid": None, "status": "error",
                    "error": "client_uuid required"}
        existing = CaseNote.objects.filter(client_uuid=cu).first()
        if existing:
            return {"client_uuid": cu, "status": "duplicate",
                    "note_id": str(existing.pk)}
        try:
            case = CaseFile.objects.select_related("consent",
                                                   "subject_person").get(
        ...
        if self._consent_frozen(case):
            return {"client_uuid": cu, "status": "error",
        ...
        occurred = parse_datetime(
            (item.get("occurred_at") or "").replace("Z", "+00:00"))
        if occurred is None:
            occurred = timezone.now()
        elif timezone.is_naive(occurred):
            occurred = timezone.make_aware(occurred)
        kind = item.get("kind") if item.get("kind") in dict(
            CaseNote.KIND_CHOICES) else "visit"
        allowed_actions = {a for a, _ in CaseNote.ACTIONS}
        actions = [a for a in (item.get("actions") or []) if a in allowed_actions]
        try:
            duration = int(item["duration_minutes"]) if item.get(
                "duration_minutes") else None
        except (ValueError, TypeError):
            duration = None
        try:
            aid_cents = int(item["aid_value_cents"]) if item.get(
                "aid_value_cents") not in (None, "") else None
        except (ValueError, TypeError):
            aid_cents = None

        note = CaseNote(case=case, author=self.membership, kind=kind,
                        occurred_at=occurred, duration_minutes=duration,
                        actions=actions, aid_value_cents=aid_cents,
                        location_kind=item.get("location_kind", "home")
                        if item.get("location_kind") in dict(
                            CaseNote.LOCATION_CHOICES) else "home",
                        client_uuid=cu)
        note.body = body[:8000]
        try:
            note.save()
            ...
            return {"client_uuid": cu, "status": "duplicate",
                    "note_id": str(existing.pk)}
        audit.emit("note.synced_offline", note, user=request.user,
                   request=request, details={"client_uuid": str(cu)})
        if item.get("finalize", True):
            note.transition_to(CaseNote.STATUS_FINAL)
            audit.emit("note.finalized", note, user=request.user, request=request)
        return {"client_uuid": cu, "status": "created", "note_id": str(note.pk),
...
class FollowUpCreateView(CommunityMixin, View):
    def post(self, request, slug, pk):
        ...
        if d.get("source_note"):
            source = CaseNote.objects.filter(pk=d["source_note"],
                                             case=case).first()
        fu = FollowUp(case=case, created_by=self.membership,
                      assigned_to=d["assigned_to"], title=d["title"].strip(),
                      due_date=d["due_date"], source_note=source)
        if d.get("detail"):
            fu.detail = d["detail"]
        fu.save()
        audit.emit("followup.created", fu, user=request.user, request=request)
        if fu.assigned_to_id != self.membership.id:
            notify(fu.assigned_to.user, "followup_assigned",
                   f"Follow-up on case {case.short_code}",
                   fu.title,
                   reverse("casework:followups-mine", kwargs={"slug": slug}))
        if _is_htmx(request):
            return render(request, "casework/_followup_row.html", {
                "community": self.community, "fu": fu,
...
class FollowUpStatusView(CommunityMixin, View):
    def post(self, request, slug, pk):
        fu = get_object_or_404(
            FollowUp.objects.select_related("case", "assigned_to"),
            pk=pk, case__community=self.community)
        allowed = (fu.assigned_to_id == self.membership.id
                   or fu.created_by_id == self.membership.id
                   or access.is_admin(self.membership))
        if not allowed:
            return _forbidden("Only the assignee, creator, or an admin "
                              "can update this follow-up.")
        new = request.POST.get("status", "")
...
class MyFollowUpsView(CommunityMixin, View):
    def get(self, request, slug):
        items = (FollowUp.objects
                 .filter(assigned_to=self.membership, status="open",
                         case__community=self.community)
                 .select_related("case").order_by("due_date"))
        ctx = {"community": self.community, "membership": self.membership,
               "items": items, "today": timezone.localdate()}
        if _is_htmx(request):
    ...
    def post(self, request, slug, pk):
        ...
        try:
            grant = CaseAccessGrant.objects.create(
                case=case, member=target, role=form.cleaned_data["role"],
                granted_by=self.membership,
                reason=form.cleaned_data["reason"].strip(),
                expires_at=form.cleaned_data["expires_at"])
        except IntegrityError:
            return _conflict("That member already has an active grant.")
        audit.emit("grant.granted", grant, user=request.user, request=request,
                   details={"member": str(target.pk),
                            "role": grant.role, "case": str(case.pk)})
        notify(target.user, "case_grant",
               f"Access granted: case {case.short_code}",
               f"{grant.role.title()} access — {grant.reason}",
               reverse("casework:detail", kwargs={"slug": slug, "pk": case.pk}))
        if _is_htmx(request):
            return render(request, "casework/_grant_row.html", {
                "community": self.community, "case": case, "grant": grant})
        return redirect("casework:detail", slug=slug, pk=case.pk)
...
class GrantRevokeView(CommunityMixin, View):
    def post(self, request, slug, pk, grant_id):
        ...
        audit.emit("grant.revoked", grant, user=request.user, request=request,
...
class CaseExportView(CommunityMixin, View):
    def get(self, request, slug, pk):
        ...
        if not (case.consent_id and case.consent_is_active()
                and "case_export" in (case.consent.scope or [])):
        ...
        for n in case.notes.filter(status=CaseNote.STATUS_FINAL).order_by(
                "occurred_at").select_related("author"):
            aid = (f" · aid ${n.aid_value_cents / 100:.2f}"
                   if n.aid_value_cents else "")
            lines += [f"--- {n.occurred_at:%Y-%m-%d %H:%M} · {n.kind} · "
                      f"{n.author.display_name}{aid}",
                      f"    actions: {', '.join(n.actions) or '—'}",
                      f"    {n.body or ''}"]
        lines += ["", "FOLLOW-UPS"]
        for fu in case.followups.all():
            lines.append(f"  [{fu.status}] {fu.title} — due {fu.due_date}")
        audit.emit("case.exported", case, user=request.user, request=request,
                   details={"scope": case.consent.scope})
        resp = HttpResponse("\n".join(lines),
                            content_type="text/plain; charset=utf-8")
        resp["Content-Disposition"] = (
            f'attachment; filename="case-{case.short_code}.txt"')
        return resp
...
class ReauthView(CommunityMixin, View):
    ...
    def get(self, request, slug):
        return render(request, "casework/reauth.html", {
            "community": self.community, "form": ReauthForm(),
    ...
    def post(self, request, slug):
        ...
        return render(request, "casework/reauth.html", {
            "community": self.community, "form": form,
            "next": request.POST.get("next", "")}, status=200)
...
class ValidateFieldView(CommunityMixin, View):
    ...
    def post(self, request, slug):
        ...
        form.is_valid()
        return render(request, "casework/_field_errors.html",
